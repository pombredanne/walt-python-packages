...
def filter_poe_rebootable(remaining_nodes, **env):
    poereboot_errors = {}
    for node in remaining_nodes.copy():
        ...
        if sw_info:
            ...
            remaining_nodes.remove(node)
    env.update(poereboot_errors = poereboot_errors,
               remaining_nodes = remaining_nodes)
    run_next_step(**env)
